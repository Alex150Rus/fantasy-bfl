...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await client.query("SELECT results.id, date, weekday, time, year, team, homeTeamGoals, guestTeamGoals FROM results INNER JOIN teams AS teams_1 ON (teams_1.id = results.guestTeamId) INNER JOIN teams AS teams_2 ON (teams_2.id = results.homeTeamId);");
